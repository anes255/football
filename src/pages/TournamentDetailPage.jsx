...
const TournamentDetailPage = () => {
  ...
  // Use backend check instead of local date comparison
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="card mb-6 bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border-yellow-500/30">
            ...
            {!user ? (
              <p className="text-gray-400">
                <Link to="/connexion" className="text-primary-400 hover:text-primary-300">Connectez-vous</Link> pour prédire le vainqueur du tournoi
              </p>
              ...
              <div className="flex items-center space-x-4 p-4 bg-white/5 rounded-xl">
                ...
                <div>
                  ...
                  {winnerPrediction.points_earned > 0 && (
                    <p className="text-sm text-green-400">+{winnerPrediction.points_earned} pts</p>
                  )}
                </div>
              </div>
            ) : canPredictWinner() ? (
              <div className="space-y-3">
                {winnerPrediction && (
                  <p className="text-sm text-green-400">✓ Prédiction actuelle: {winnerPrediction.team_name}</p>
                )}
              ...
              <div>
                {winnerPrediction ? (
                  ...
                  <p className="text-gray-400">Le tournoi a commencé, les prédictions sont fermées.</p>
                )}
              </div>
            )}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {Object.keys(groupedTeams).length === 0 ? (
              <div className="card text-center py-12">
                ...
                <p className="text-gray-400">Aucune équipe assignée aux groupes</p>
              </div>
            ) : (
