...
const AdminScoring = () => {
  ...
  const fetchRules = async () => { 
    try { 
      const r = await scoringAPI.getRules(); 
      const obj = {}; 
            ...
            {ruleCards.map((rule, i) => (
              <motion.div key={rule.key} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="card">
                <div className="flex items-start space-x-4">
                  <div className={`p-3 rounded-xl bg-${rule.color}-500/20`}>
                    <rule.icon className={`w-6 h-6 text-${rule.color}-500`} />
                  </div>
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-4">
                      <input 
                        ...
                        className="w-24 input text-center text-xl font-bold" 
                      />
