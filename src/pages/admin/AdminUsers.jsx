...
const AdminUsers = () => {
  ...
  const toggleAdmin = async (user) => { 
    try { 
      ...
      toast.success(`${user.name} ${!user.is_admin ? 'est maintenant admin' : 'n\'est plus admin'}`); 
      fetchUsers(); 
    } catch (e) { 
      toast.error('Erreur'); 
    } 
    ...
    <div className="min-h-screen pt-16 flex">
      ...
      <div className="flex-1 p-8">
        ...
        <div className="relative mb-6">
          ...
          <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Rechercher..." className="input pl-10" />
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          ...
          <div className="card text-center">
            ...
            <p className="text-2xl font-bold text-white">{users.reduce((s, u) => s + (u.total_predictions || 0), 0)}</p>
          ...
          <div className="card overflow-hidden">
            <table className="w-full">
              <thead className="bg-white/5">
                <tr>
                  <th className="px-4 py-3 text-left text-sm text-gray-400">Nom</th>
                  <th className="px-4 py-3 text-left text-sm text-gray-400">Téléphone</th>
                  <th className="px-4 py-3 text-center text-sm text-gray-400">Points</th>
                  <th className="px-4 py-3 text-center text-sm text-gray-400">Pronostics</th>
                  <th className="px-4 py-3 text-center text-sm text-gray-400">Inscrit</th>
                  <th className="px-4 py-3 text-center text-sm text-gray-400">Admin</th>
                  ...
                  <motion.tr key={user.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-white/5">
                    <td className="px-4 py-3">
                      <div className="flex items-center space-x-3">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary-500 to-secondary-600 flex items-center justify-center text-sm font-bold">
                    ...
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => toggleAdmin(user)} className={`px-3 py-1 rounded-full text-xs font-medium ${user.is_admin ? 'bg-red-500/20 text-red-400' : 'bg-gray-500/20 text-gray-400'}`}>
                        {user.is_admin ? 'Admin' : 'User'}
                      </button>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => handleDelete(user.id)} className="p-2 rounded-lg bg-red-500/20 text-red-400 hover:bg-red-500/30">
