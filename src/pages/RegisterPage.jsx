...
import { User, Phone, Lock, Globe, UserPlus, Flag } from 'lucide-react';
...
const RegisterPage = () => {
  ...
  const renderFlag = (flagUrl) => {
    if (flagUrl && (flagUrl.startsWith('data:') || flagUrl.startsWith('http'))) {
      return <img src={flagUrl} alt="" className="w-full h-full object-cover" />;
    }
    return <span className="text-xl">{flagUrl || '🏳️'}</span>;
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="input pl-10" placeholder="Votre nom" required />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
                <input type="password" value={formData.confirmPassword} onChange={(e) => setFormData({ ...formData, confirmPassword: e.target.value })} className="input pl-10" placeholder="••••••••" required />
              </div>
            </div>
            ...
            <div>
              ...
              {loadingTeams ? (
                <div className="input flex items-center justify-center">
                  ...
                  <span className="ml-2 text-gray-400">Chargement des équipes...</span>
                </div>
              ) : teams.length > 0 ? (
                <select 
                ...
                <div className="input text-gray-500 text-sm flex items-center">
                  <Flag className="w-4 h-4 mr-2" />
                  Aucune équipe disponible pour le moment
                </div>
              )}
              <p className="text-xs text-gray-500 mt-1">Points bonus si votre équipe remporte le tournoi !</p>
            </div>
            ...
            {selectedTeam && (
              <motion.div 
                ...
                className="flex items-center space-x-3 p-4 bg-primary-500/10 rounded-xl border border-primary-500/30"
              >
                <div className="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center overflow-hidden">
                  {renderFlag(selectedTeam.flag_url)}
                </div>
                <div>
                  <p className="text-white font-semibold">{selectedTeam.name}</p>
                  <p className="text-xs text-primary-500">Votre équipe favorite ⭐</p>
                </div>
              </motion.div>
            )}
            
            <button type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center space-x-2 mt-6">
              {loading ? <div className="animate-spin w-5 h-5 border-2 border-white border-t-transparent rounded-full" /> : <><UserPlus className="w-5 h-5" /><span>S'inscrire</span></>}
            </button>
          </form>
